...
import HalamanEntriMenu, { MenuForm, defaultMenuForm } from './HalamanEntriMenu';
...
export default function HalamanDaftarMenu() {
  ...
  const [form, setForm] = useState<MenuForm>(defaultMenuForm);
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full animate-scale-up">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 py-2.5 rounded-xl bg-red-500 text-white font-semibold shadow-lg shadow-red-500/30"
            >
              Hapus
            </button>
          </div>
  ...
  const tampilModalMenu = () => {
    if (!modalOpen) return null;

    return (
      <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/50 px-4">
        <div className="bg-white rounded-3xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto animate-slide-up">
          <HalamanEntriMenu
            form={form}
  ...
  const tampilHalamanDaftarMenu = () => {
    if (loading) {
      return (
        <AdminLayout title="Halaman Daftar Menu">
        ...
        <div className="bg-white rounded-2xl shadow p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 bg-brand-accent text-brand-dark font-semibold px-4 py-2 rounded-xl shadow shadow-brand-accent/30"
            >
              ...
              <tbody>
                {visibleMenus.map((menu) => (
                  <tr key={menu.id_menu} className="border-b border-slate-100">
                    ...
                    <td className="py-4 pr-4 align-top">
                      <div className="font-semibold text-brand-dark">{menu.nama_menu}</div>
                      <p className="text-xs text-slate-500 whitespace-pre-line">{forceWrapText(menu.deskripsi_menu ?? '')}</p>
                    </td>
                    ...
                    <td className="py-4 pl-4 align-top font-semibold text-brand-dark">{formatCurrency(menu.harga_menu)}</td>
                    ...
                    <td className="py-4 align-top text-right pr-2">
                      <div className="inline-flex items-center gap-2">
                        <button
                          className="px-3 py-1 rounded-lg border border-slate-200 text-xs font-semibold"
                          onClick={() => handleEdit(menu)}
                        >
                          Edit
                        </button>
                        <button
                          className="px-3 py-1 rounded-lg border border-red-200 text-xs font-semibold text-red-500"
  ...
  return tampilHalamanDaftarMenu();
}

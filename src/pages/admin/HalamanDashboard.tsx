            ...
            <div className="flex gap-3">
              {quickActions.map((action) => (
                <Link
                  key={action.href}
                  href={action.href}
                  className={`${action.color} text-white px-4 py-2 rounded-xl shadow-lg shadow-gray-200 flex items-center gap-2 text-sm font-semibold`}
                >
                ...
                {stats.chartData.length > 0 ? (
                  <div className="w-full h-full relative flex items-end justify-between gap-1">
                    {stats.chartData.map((d, i) => {
                      const maxRev = Math.max(...stats.chartData.map((entry) => entry.revenue)) || 1;
